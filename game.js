  ...
  doneBtn.addEventListener("click", async () => {
    ...
    if (oppKey && g[oppKey] && g[oppKey].ready) {
      ...
      if (!g.turn) {
        ...
        // show a local popup immediately (so player sees who starts)
        try { showTurnPopup(first); } catch (e) { /* ignore */ }
      }
      phase = "playing";
...
// SMALLER CELLS: kleiner maken zodat twee borden naast elkaar passen
// Origineel was 40px — we verkleinen naar 28px
function createBoard(container, gridSize, clickable = false, handler = null) {
  container.innerHTML = "";
  const CELL_PX = 28; // <<< hier aangepast
...
onValue(gameRef, async (snap) => {
  ...
  if (self && opp && Array.isArray(self.ships) && Array.isArray(opp.ships) && self.ready && opp.ready) {
    ...
    if (!data.turn) {
      const first = Math.random() < 0.5 ? username : opponent;
      await update(gameRef, { turn: first });
      // show a local popup immediately so players see who starts
      try { showTurnPopup(first); } catch (e) { /* ignore */ }
    }
    phase = "playing";
    gameNote.textContent = "Spel gestart!";
