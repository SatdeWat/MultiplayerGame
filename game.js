...
function placeShip(x, y, cell) {
  ...
  if (myShips.length === shipsCount) {
    gameMessage.textContent = "Klik op Klaar als je klaar bent";
  }
}

// --- Klaar knop ---
readyBtn.addEventListener("click", async () => {
  if (myShips.length !== shipsCount) { alert(`Plaats eerst al je ${shipsCount} schepen!`); return; }
  phase = "waiting";
  gameMessage.textContent = "Wachten op tegenstander...";

  if (!isGuest) {
    await set(ref(db, `games/${lobbyCode}/${username}`), { ships: myShips, shots: {} });
  }

  checkBothReady();
});

// --- Check of beide spelers klaar ---
function checkBothReady() {
  onValue(gameRef, snapshot => {
    ...
    const ready = players.every(p => data[p] && data[p].ships && data[p].ships.length > 0);
    if (ready) startGame(players);
  });
}
